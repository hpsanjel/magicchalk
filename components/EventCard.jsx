...
const EventCard = ({ date, title, time, venue }) => {
	...
	return (
		<div className="relative flex flex-col justify-between bg-white/70 backdrop-blur-md border border-gray-200 shadow-xl rounded-2xl p-6 h-72 w-72 mx-auto transition-transform duration-300 hover:scale-105 hover:shadow-2xl group overflow-hidden">
			{/* Accent Bar */}
			<div className="absolute left-0 top-0 h-full w-2 bg-gradient-to-b from-green-400 to-green-700 rounded-l-2xl" />

			{/* Date Section */}
			<div className="flex items-center gap-4 mb-4 z-10">
				<div className="w-16 h-16 bg-gradient-to-br from-green-400 to-green-700 rounded-xl flex flex-col items-center justify-center shadow-lg border-4 border-white group-hover:scale-110 transition-transform">
				...
				<div className="flex-1">
					<h3 className="text-lg font-bold text-gray-800 group-hover:text-green-700 transition-colors line-clamp-2">{title}</h3>
				</div>
			</div>
			...
			<div className="absolute -bottom-10 -right-10 w-32 h-32 bg-green-100 rounded-full blur-2xl opacity-60 z-0" />
		</div>
	);
};
